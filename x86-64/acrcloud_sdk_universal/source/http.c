...
void post(acrcloud_config* mconfig, 
    ...
    char acr_url[2048];
    sprintf(acr_url, "http://%s%s", mconfig->host_, http_endpoint);
    LOGDBG("%s\n", acr_url);
    ...
    curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS, mconfig->timeout_ms_);
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT_MS, mconfig->timeout_ms_);
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1);
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, _process_curl_result_data);
    if (mconfig->curl_proxy_address_) {
        curl_easy_setopt(curl, CURLOPT_PROXY, mconfig->curl_proxy_address_);
    }
